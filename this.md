...
一句话定义js中this关键字：
```包含this关键字的函数作为方法被调用时所属的对象。```

这句话可以分3部分来理解:
1、包含this关键字的函数。2、作为方法被调用时。3、所属的对象。
...
非严格模式，如果函数是独立调用的（不是作为对象方法、不是通过 new、不是通过 call/apply/bind 等），那么函数内的 this 会默认指向全局对象（浏览器中的 window，Node.js 中的 global）。
...
严格模式下，不再有默认的全局对象绑定。如果函数是独立调用的，那么进入函数执行上下文时，this 的值就是 undefined，而不会被隐式替换成全局对象。
```
function f2(){
	"use strict"; // 这里是严格模式
	return this;
